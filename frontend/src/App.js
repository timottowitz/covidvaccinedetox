  ...
  const submit = async () => {
    ...
    try{
      ...
      for(const f of files){
        ...
        const { data } = await axios.create({ baseURL: API }).post('/resources/upload', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
...
function Treatments(){
  const api = useApi();
  const [items, setItems] = useState([]);
  useEffect(()=>{ (async()=>{ try{ const {data} = await api.get('/treatments'); setItems(data);} catch(e){ toast({title:'Load failed', description:'Could not load treatments'}); } })(); },[]);
  return (
    <>
      <Header />
      <div className="container">
            ...
            <Card key={t.id} className="card">
              ...
              <CardContent>
                <ul style={{marginBottom:8, paddingLeft:18}}>
                  {(t.mechanisms||[]).map((m,i)=>(<li key={i}>{m}</li>))}
                </ul>
                {t.dosage && <div className="card-meta">Dosage: {t.dosage}</div>}
                {t.duration && <div className="card-meta">Duration: {t.duration}</div>}
                <div className="card-actions" style={{marginTop:8}}>
                  {(t.links||[]).map((l,i)=>(<a key={i} className="pill" href={l} target="_blank" rel="noreferrer">Study {i+1}</a>))}
                </div>
              </CardContent>
            </Card>
          ))}
